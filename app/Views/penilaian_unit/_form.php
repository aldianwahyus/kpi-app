...
<script>
document.querySelectorAll('.realisasi-input').forEach(function (input) {
    input.addEventListener('change', function () {
        const row        = this.closest('.kpi-row');
        const kpiId       = row.getAttribute('data-kpi');
        const pegawaiId   = '<?= (int)$pegawai['id'] ?>';
        const realisasi   = this.value;

        if (realisasi === '') return;

        const skorOutput  = row.querySelector('.skor-output');
        const kontribOutput = row.querySelector('.kontribusi-output');

        fetch('<?= base_url('penilaian/ajaxHitung') ?>', {
            method: 'POST',
            headers: {
        ...
        .then(function (data) {
            if (!data.valid) {
                console.warn('Gagal hitung:', data.message);
                return;
            }

            skorOutput.value     = data.skor;
            kontribOutput.value  = data.kontribusi;
            ...
            skorOutput.style.borderColor = colors[data.color] || '#ced4da';
            skorOutput.style.color       = colors[data.color] || '#333';
        })
        .catch(function (err) {
