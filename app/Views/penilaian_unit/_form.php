...
<div class="row g-3 mb-3">
  <div class="col-md-4">
    <div class="stat-card text-center">
      <?php
      $rataCapaian = count($existing) > 0
          ? round(array_sum(array_column($existing,'capaian'))
                  / count($existing) * 100, 2)
          : 0;
      ?>
      <div class="stat-value" style="color:#2E75B6">
        <?= $rataCapaian > 0 ? number_format($rataCapaian,2).'%' : '—' ?>
      </div>
  ...
  <div class="col-md-4">
    <div class="stat-card text-center">
      <?php
      $grade_unit = '';
      if ($rataCapaian >= 91)     $grade_unit = 'A';
      elseif ($rataCapaian >= 76) $grade_unit = 'B';
      elseif ($rataCapaian >= 61) $grade_unit = 'C';
      elseif ($rataCapaian >= 41) $grade_unit = 'D';
      elseif ($rataCapaian > 0)   $grade_unit = 'E';
      $gcu = match($grade_unit) {
          'A' => ['#C6EFCE','#375623'],
          'B' => ['#BDD7EE','#1F4E79'],
          'C' => ['#FFF2CC','#7F6000'],
          'D' => ['#FCE4D6','#C00000'],
      ...
      <div class="stat-value"
           style="color:<?= $gcu[1] ?>;background:<?= $gcu[0] ?>;
  ...
  <div class="col-md-4">
    <div class="stat-card">
      ...
      <?php $pct = $totalKpi > 0
          ? round(count($existing)/$totalKpi*100) : 0; ?>
      <div class="progress" style="height:8px">
        <div class="progress-bar bg-success"
             style="width:<?= $pct ?>%"></div>
      </div>
      <div style="font-size:11px;color:#6B7280;margin-top:4px">
        <?= count($existing) ?> dari <?= $totalKpi ?> KPI diisi
      </div>
    </div>
  </div>
</div>
...
<script>
const BASE_URL_UNIT  = '<?= base_url() ?>';
const CSRF_NAME_UNIT = '<?= csrf_token() ?>';
// Token CSRF disimpan sebagai 'let' (bukan 'const') karena harus
// diperbarui setelah setiap permintaan AJAX berhasil — Config\Security
// ::$regenerate = true membuat token berubah setiap kali diverifikasi,
// sehingga nilai awal saat halaman dimuat akan menjadi usang setelah
// permintaan pertama dan membuat permintaan kedua dst. ditolak (403).
let CSRF_HASH_UNIT = '<?= csrf_hash() ?>';

document.querySelectorAll('.kpi-unit-input').forEach(function (input) {
    input.addEventListener('change', function () {
        ...
        if (target === '' || realisasi === '' || parseFloat(target) <= 0 || parseFloat(realisasi) <= 0) {
            return;
        }

        const capaianBadge = document.getElementById('capaian_' + kpiId);

        const params = new URLSearchParams();
        params.append(CSRF_NAME_UNIT, CSRF_HASH_UNIT);
        params.append('kpi_id', kpiId);
        params.append('target', target);
        params.append('realisasi', realisasi);

        fetch(BASE_URL_UNIT + 'penilaian-unit/ajax-hitung', {
            method: 'POST',
            headers: {
        ...
        .then(function (data) {
            ...
            capaianBadge.textContent = data.pct ?? '—';
            ...
            capaianBadge.style.background = c.bg;
            capaianBadge.style.color      = c.color;
        })
        .catch(function (err) {
            console.error('Error hitung capaian KPI Unit:', err);
        });
    });
});
</script>